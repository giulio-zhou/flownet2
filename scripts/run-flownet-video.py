...
hdf5_file.create_dataset('data', [args.num_frames - 1, height, width, 2], 'f4')
for j in range(0, args.num_frames - 1, args.batchsize):
    start, end = j, min(j + args.batchsize, args.num_frames - 1)
    imgs = [imgs[-1]] + [next(vreader) for _ in range(start, end)]
    ...
    blob = blob[:end-start]
    hdf5_file['data'][j:j+end-start] = np.copy(blob)
